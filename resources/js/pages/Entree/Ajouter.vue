    ...
    <div class="md-layout">
      <div class="md-layout-item md-medium-size-100 md-size-66">
                ...
                <div class="md-layout-item md-small-size-100 md-size-33">
                  <md-field>
                    <label
                      style="font-family: 'Droid Arabic Naskh', serif;font-weight: regular;"
                    >ثمن البيع</label>
                    <md-input type="text" v-model="ligne.prix_vente"></md-input>
                  </md-field>
                </div>
                <div class="md-layout-item md-small-size-100 md-size-33">
                  <md-field>
        ...
        <mdb-modal :show="modalEchange" @close="modalEchange = false">
          ...
          <mdb-modal-body>
            <p>هذا المورد يدين لك ب {{sommeFournisseur}} درهم وأنت اشتريت من عنده بضاعة بقيمة {{Entree.montant_total}} درهم</p>
...
export default {
  ...
  methods: {
    ...
    onExit() {
      //this.$router.push({ name: "entrees" });
       fetch(`/api/sommeFourisseur/${this.Entree.fournisseur.id}?token=` + this.token)
        .then(res => res.json())
        .then(res => {
          this.sommeClient = res.sommeClient;
          this.sommeFournisseur = res.sommeFournisseur;
          if ( this.sommeFournisseur >= this.sommeClient) {
    ...
    ok() {
      if (this.sommeFournisseur >= this.Entree.montant_total) {
        this.Entree.montant_paye = this.Entree.montant_total;
      } else if (this.sommeFournisseur < this.Entree.montant_total) {
        this.Entree.montant_paye = this.sommeFournisseur;
      }
      fetch(`api/entree/${this.Entree.id}`, {
        method: "put",
        body: JSON.stringify(this.Entree),
        ...
        .then(data => {
          ...
          let AvanceEntree = {
            ...
            description: "مبادلة",
            montant: this.Entree.montant_paye
          };
          fetch("api/AvanceEntree?token=" + this.token, {
            method: "post",
            body: JSON.stringify(AvanceEntree),
            headers: {
              "content-type": "application/json"
            }
          })
